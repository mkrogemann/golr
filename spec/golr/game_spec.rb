...
module Golr
  describe Game do
    ...
    context '#wrap_around_board_edges' do
      it "adjusts x and y if required" do
        game = Game.new(5,5)

        game.send(:wrap_around_board_edges, Key.key(-1,5)).should == Key.key(5,5)
        game.send(:wrap_around_board_edges, Key.key(0,0)).should == Key.key(5,5)
        game.send(:wrap_around_board_edges, Key.key(6,5)).should == Key.key(1,5)
        game.send(:wrap_around_board_edges, Key.key(3,6)).should == Key.key(3,1)
      end
    end
